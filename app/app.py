...
# Configure BEFORE importing any ML libraries
# This helps reduce memory earlier in the process
os.environ['CUDA_VISIBLE_DEVICES'] = ''  # Force CPU, no GPU memory

# =============================================================================
# LAZY IMPORTS - Only load ML libraries when needed (reduces startup memory)
# =============================================================================
# Instead of importing at startup, we import inside each route function.
# This significantly reduces initial memory usage on Render's free tier (512MB).

app = None
predict_module = None

def _get_predict_module():
    """Lazy import of ML functions - only loads when first request arrives."""
    global predict_module
    if predict_module is None:
        # Import only when first needed - saves memory at startup
        from src.api import predict as pm
        predict_module = pm
    return predict_module

def create_app():
    """Create Flask app - models not loaded until first request."""
    global app
    ...
    def predict_image_api():
        ...
        try:
            pm = _get_predict_module()
            result = pm.predict_image(filepath)
    ...
    def predict_tabular_api():
        ...
        try:
            pm = _get_predict_module()
            result = pm.predict_tabular(data)
            return jsonify(result)
        except Exception as e:
    ...
    def predict_complete_api():
        ...
        Uses lazy loading to minimize memory at startup.
        """
        data = request.json
        image_result = None
        tabular_result = None
        ...
        # Process chest X-ray - lazy loads model
        if data.get('image_data'):
            try:
                ...
                pm = _get_predict_module()
                image_result = pm.predict_image(filepath)
        ...
        # Process tabular data - lazy loads model
        if data.get('tabular_data'):
            try:
                pm = _get_predict_module()
                tabular_result = pm.predict_tabular(data['tabular_data'])
        ...
        # Validate predictions before LLM
        ...
        if image_valid or tabular_valid:
            try:
                ...
                pm = _get_predict_module()
                llm_analysis = pm.generate_medical_report(
                    image_result, 
                    tabular_result,
...
# Create app lazily - models load on first request, not at startup
app = create_app()
...
if __name__ == '__main__':
    ...
    print("[INFO] Models will load on first request (lazy loading enabled)")
